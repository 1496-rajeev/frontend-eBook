...
import { cardDataReact, topicWiseDataReact } from "../../jsonData/reactjs";

export default function ReactJs() {
  const [topicId, setTopicId] = useState<number | null>(null);
  const [searchTerm, setSearchTerm] = useState("");
  const filteredData = topicWiseDataReact.find((topic) => topic.id === topicId);

  const filteredCards = cardDataReact.filter((card) =>
        ...
        {topicId === null ? (
          <div className="mt-4">
            ...
            <CardList
              ...
              cardData={
                filteredCards.length !== 0 ? filteredCards : cardDataReact
              }
            />
          </div>
        ) : (
